...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first) {
    ...
    if (Array.isArray(obj)) {
        ...
        for (var name in obj) {
            var child = obj[name];
            var attrs = self.generateAttributes(child);
            var obj_ns = name.match(/:/) ? '' : ns;
            parts.push(['<',obj_ns,name,xmlnsAttrib+attrs,'>'].join(''));
            if (child[WSDL.contentKey]) {
              parts.push(xmlEscape(child[WSDL.contentKey]));
            } else {
              parts.push(self.objectToXML(child, name, namespace, xmlns));
            }
            parts.push(['</',obj_ns,name,'>'].join(''));
        }
    }
    else if (obj) {
...
WSDL.attributeKey = '@';
WSDL.contentKey   = 'value';

WSDL.prototype.generateAttributes = function(obj) {
    var self = this, attributes = [];
    for (name in obj ) {
        if (name[0] === WSDL.attributeKey) {
            attributes.push(xmlEscape(name.substring(1))+'="'+xmlEscape(obj[name])+'"');
            delete obj[name];
        }
    }
