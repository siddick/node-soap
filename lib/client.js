...
Client.prototype.addHttpHeader = function(exheaders){
    if(!this.httpHeaders){
        this.httpHeaders = {};
    }
    for (var attr in exheaders) { this.httpHeaders[attr] = exheaders[attr]; }
}
...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    if (self.httpHeaders)
        for (var attr in self.httpHeaders) { headers[attr] = self.httpHeaders[attr] }
